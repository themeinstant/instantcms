...
function applet_menu(){
	...
	if ($do == 'update'){
        ...
        if (isset($_REQUEST['id'])) {
			...
			if (!$is_public){
				$access_list = cmsCore::request('allow_group', 'array_int');
                ...
                <td valign="top">
                    ...
                    <div>
                        <select name="mode" id="linktype" style="width:100%" onchange="showMenuTarget()">
                            ...
                            <option value="pricecat" <?php if (@$mod['linktype']=='pricecat') { echo 'selected="selected"'; }?>>Открыть категорию прайс-листа</option>
                    ...
                    {tab=Публикация}
                        ...
                        <div>
                            <select name="template" id="template" style="width:100%">
                                <option value="0" <?php if (@$mod['template']==0 || !$mod['template']) { echo 'selected="selected"'; } ?>>По-умолчанию</option>
                                <?php
