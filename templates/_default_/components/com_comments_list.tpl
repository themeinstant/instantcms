			...
			<tr>
				...
				{/if}
                    ...
                    <div style="margin-top:15px;">
                        <a href="javascript:void(0)" onclick="addComment('{$target|escape:'html'}', '{$target_id}', {$comment.id})" class="ajaxlink">{$LANG.REPLY}</a>
                        {if $is_user}
                            {if $is_admin || ($comment.is_my && $comment.is_editable && $comment.content_bbcode) || ($user_can_moderate && $comment.content_bbcode)}
                                {if !$comment.content_bbcode}
                                    | <a href="/admin/index.php?view=components&do=config&link=comments&opt=edit&item_id={$comment.id}">{$LANG.EDIT}</a>
                                {else}
                                    | <a href="javascript:" onclick="editComment('{$comment.id}', '{csrf_token}')" class="ajaxlink">{$LANG.EDIT}</a>
                                {/if}
                            {/if}
                            {if $is_admin || ($comment.is_my && $user_can_delete) || $user_can_moderate}
                                | <a href="javascript:" onclick="deleteComment({$comment.id}, '{csrf_token}'{if $comments[$next].level > $comment.level}, 1{/if});return false;" class="ajaxlink">{if $comments[$next].level > $comment.level}{$LANG.DELETE_BRANCH}{else}{$LANG.DELETE}{/if}</a>
                            {/if}
                        {/if}
                    </div>
