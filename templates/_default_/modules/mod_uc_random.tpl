{if $is_uc}
	{foreach key=tid item=item from=$items}
		<div align="center" id="uc_random_img"><a href="/catalog/item{$item.id}.html"><img src="/images/catalog/small/{$item.imageurl}.jpg" border="0"/></a></div>
		...
		{if $cfg.showtitle}
			...
			{if $item.viewtype == 'shop'}	
				<div style="margin-bottom:10px" align="center" id="uc_random_price">{$item.price} {$LANG.UC_RANDOM_RUB}</div>
			{/if}
		{/if}
