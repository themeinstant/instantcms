...
////////////////////////////////////////////////////////////////////////////////
    ...
    public function cacheThreadLastPost($thread_id) {
        ...
        $yaml_post = cmsDatabase::escape_string(cmsCore::arrayToYaml($post));
...
////////////////////////////////////////////////////////////////////////////////
    ...
    public function getForums($is_admin=false){
        ...
        $sql = "SELECT f.*, cat.title as cat_title, cat.seolink as cat_seolink
                FROM cms_forums f
                LEFT JOIN cms_forum_cats cat ON cat.id = f.category_id AND cat.published = 1
                WHERE f.parent_id > 0 {$pub_sql}
                {$this->inDB->where}
		...
		while ($forum = $this->inDB->fetch_assoc($result)){
            ...
            $first_level = isset($first_level) ? $first_level : $forum['NSLevel'];
            ...
            if($forum['NSLevel'] == $first_level){
                $forums[] = $forum;
            } else { // формируем подфорумы
                $forums[end(array_keys($forums))]['sub_forums'][] = $forum;
            }

        }

        return cmsCore::callEvent('GET_FORUMS', $forums);
...
////////////////////////////////////////////////////////////////////////////////
    public function addPost($post){
        ...
        if($last_post['user_id'] == $post['user_id'] && $minutes_passed < 20){
            ...
            $u_post['content']      = cmsDatabase::escape_string($last_post['content']."\r\n".$post['content']);
            $u_post['content_html'] = cmsDatabase::escape_string($last_post['content_html'].'<em class="added_later">'.
                                        $_LANG['ADDED_LATER'].
                                        ($minutes_passed ?
...
////////////////////////////////////////////////////////////////////////////////
    ...
    public function addPoll($poll, $thread_id){
        ...
        $t_poll['answers']   = cmsDatabase::escape_string(cmsCore::arrayToYaml($answers));
