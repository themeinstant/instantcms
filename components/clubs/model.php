   ...
   public function getVisibility($target, $target_id) {
        ...
        switch($target){
            ...
            case 'club_post':
						...
						$club_id = $this->inDB->get_fields('cms_blogs', "id='{$post['blog_id']}'", 'user_id');
					    $clubtype = $this->inDB->get_field('cms_clubs', "id='{$club_id}'", 'clubtype');
					    if($clubtype == 'private') { $is_hidden = 1; }
                        break;
...
/* ==================================================================================================== */
    ...
    public function updateClub($club_id, $item){
        ...
        $sql = "UPDATE cms_blogs SET allow_who = '{$blog_allow_who}' WHERE user_id = '{$club_id}' AND owner = 'club' LIMIT 1";
